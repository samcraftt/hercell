...
def extract_dab_map(pil_img):
    """
    Extract DAB/brown stain channel from RGB image.
    Returns a 0-1 numpy array.
    """
...
# ----------------------------
# Dataset: returns image, label, DAB map
# ----------------------------
...
class BinaryHER2DABDataset(Dataset):
    ...
    def augment(self, img):
        """
        Simple spatial augmentation that applies to both image and DAB
        because DAB is extracted after augmentation.
        """
        ...
        # Rotate by multiples of 90 to avoid weird interpolation artifacts
    ...
    def __getitem__(self, idx):
        ...
        dab_tensor = torch.from_numpy(dab).unsqueeze(0)  # [1, H, W]

        return img_tensor, torch.tensor(label).float(), dab_tensor, str(path)


# ----------------------------
# DAB-regularized heatmap model
# ----------------------------

class DABRegularizedHeatmapConvNeXt(nn.Module):
    def __init__(self, topk_frac=0.10):
        ...
        # Earlier features give higher-resolution maps than final ConvNeXt features.
    ...
    def forward(self, x):
        features = self.features(x)                    # [B, 384, H, W]
        heatmap_logits = self.heatmap_head(features)   # [B, 1, H, W]

        # Top-k pooling: focus classification on the hottest regions.
...
def compute_loss(image_logits, heatmap_logits, labels, dab_maps):
    ...
    Total loss =
      classification loss
      + DAB alignment loss on positive images
      + sparsity penalty on negative images
    """
    ...
    # Positive images: encourage learned heatmap to overlap DAB/brown signal.
    # Weight brighter DAB pixels more strongly.
    ...
    # Negative images: discourage random broad activation.
    ...
    return total_loss, cls_loss.item(), align_loss.item(), negative_sparsity_loss.item()
...
def train_one_epoch(model, loader, optimizer):
    ...
    for images, labels, dab_maps, paths in tqdm(loader, desc="Training"):
        ...
        loss, cls_loss, align_loss, sparse_loss = compute_loss(
            image_logits,
            heatmap_logits,
            labels,
            dab_maps,
        ...
        total_cls += cls_loss * images.size(0)
        total_align += align_loss * images.size(0)
        total_sparse += sparse_loss * images.size(0)
...
def main():
    ...
    model = DABRegularizedHeatmapConvNeXt(topk_frac=0.10).to(DEVICE)

    optimizer = torch.optim.AdamW(model.parameters(), lr=LR, weight_decay=1e-4)

    best_val_acc = 0.0
    best_model_path = SAVE_DIR / "dab_regularized_heatmap_convnext_best.pt"
    ...
    for epoch in range(EPOCHS):
        ...
        if val_stats["acc"] > best_val_acc:
            ...
            print("Saved best DAB-regularized heatmap model.")
    ...
    with open(PROJECT_ROOT / "dab_regularized_heatmap_test_predictions.json", "w") as f:
        json.dump(test_stats, f, indent=2)

    print("Saved predictions to dab_regularized_heatmap_test_predictions.json")
